-- Migration 68: una sola limpieza por día para habitaciones que NO requieren limpieza.
--
-- El barrido diario de habitaciones vacías registraba varias marcas por pieza el mismo día
-- (se vieron 4 filas para una misma habitación). El dueño quiere permitir marcar la limpieza
-- de una habitación que no requiere limpieza (vacía) UNA sola vez por día. Las que sí
-- requieren limpieza (check-out en estado 'cleaning', mantenimiento, u ocupadas overnight)
-- no cambian: pueden marcarse cuando corresponda.
--
-- Solo se agrega el guard de "ya limpiada hoy" para el caso NO-requerido; el resto de
-- rpc_mark_room_clean es idéntico a la migración 67.
...
BEGIN
  ...
  SELECT r.status, r.room_number INTO v_current_status, v_room_number
  ...
  IF v_input_cleaning_type IS NOT NULL
     AND v_input_cleaning_type NOT IN ('habitacion_ocupada','limpieza_mantenimiento','limpia_ocupada','limpia_vacia','limpia_repaso') THEN
    RAISE EXCEPTION 'Tipo de limpieza invalido.' USING errcode = '22023';
  END IF;

  SELECT EXISTS (
    SELECT 1 FROM public.room_cleaning_log l
    WHERE l.room_id = p_room_id AND l.cleaned_at >= v_day_start AND l.cleaned_at < v_day_end
      AND l.cleaning_category = 'checkin_daily'
  ) INTO v_expected_cleaned_today;

  SELECT EXISTS (
    SELECT 1 FROM public.reservations res
    WHERE res.room_id = p_room_id AND res.status IN ('checked_in','checked_out')
      AND res.actual_check_in IS NOT NULL AND res.actual_check_in < v_day_start
      AND COALESCE(res.actual_check_out, 'infinity'::timestamptz) > v_day_start
  ) INTO v_had_real_stay_at_cut;

  SELECT EXISTS (
    SELECT 1 FROM public.reservations res
    WHERE res.room_id = p_room_id AND res.status = 'checked_in'
  ...
  v_requires_cleaning := v_current_status IN ('cleaning','maintenance')
    OR (v_had_real_stay_at_cut AND NOT v_expected_cleaned_today);

  -- Una sola limpieza por dia para habitaciones que NO requieren limpieza.
  IF NOT v_requires_cleaning THEN
    SELECT EXISTS (
      SELECT 1 FROM public.room_cleaning_log l
      WHERE l.room_id = p_room_id AND l.cleaned_at >= v_day_start AND l.cleaned_at < v_day_end
    ) INTO v_already_today;
    IF v_already_today THEN
      RAISE EXCEPTION 'Ya se registro la limpieza de hoy para esta habitacion.' USING errcode = '22023';
    END IF;
  END IF;
  ...
  SELECT full_name INTO v_cleaner_name FROM public.profiles WHERE id = v_user_id;

  INSERT INTO public.room_cleaning_log (room_id, cleaned_by, cleaner_name, previous_status, notes, cleaning_type, cleaning_category, outcome)
  VALUES (p_room_id, v_user_id, v_cleaner_name, v_current_status::text, NULLIF(BTRIM(p_notes), ''), v_effective_cleaning_type, v_category, 'cleaned')
  ...
  IF v_category = 'occupied_anomaly' THEN
    ...
    VALUES ('room_occupied_without_active_reservation',
      format('Se registro "Habitacion estaba ocupada" en la Hab. %s sin reserva activa que lo justifique. Limpio: %s.', v_room_number, COALESCE(v_cleaner_name, 'desconocido')),
      p_room_id, v_cleaning_log_id);
  ...
  RETURN jsonb_build_object(
    'room_id', p_room_id, 'cleaned_by', v_user_id, 'previous_status', v_current_status,
    'new_status', CASE WHEN v_current_status IN ('cleaning','maintenance') THEN 'available' ELSE v_current_status::text END,
    'requires_cleaning', v_requires_cleaning, 'cleaning_type', v_effective_cleaning_type,
    'cleaning_category', v_category, 'outcome', 'cleaned',
    'alert_generated', v_alert_generated, 'cleaning_log_id', v_cleaning_log_id
  );
END;
$$;
