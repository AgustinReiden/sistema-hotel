-- Migration 57: Nombre y apellido separados + DNI obligatorio para cliente ocasional
--
-- 1) Agrega columnas client_first_name / client_last_name a reservations (nullables; las
--    filas viejas quedan en null y la UI cae al client_name compuesto).
-- 2) Re-crea rpc_staff_assign_walk_in (vigente: migracion 54) agregando p_client_dni,
--    p_client_first_name y p_client_last_name. Antes el walk-in ocasional NO guardaba DNI.
-- 3) Re-crea rpc_staff_create_reservation (vigente: migracion 54) agregando
--    p_client_first_name y p_client_last_name.
--
-- En modo manual el nombre completo (client_name) se compone como "Nombre Apellido" para
-- compatibilidad de display; se mantiene el fallback a p_client_name si first/last vienen null.
-- Como cambian las firmas, se hace DROP de las versiones anteriores antes del CREATE.
...
-- ---------------------------------------------------------------------------
-- rpc_staff_assign_walk_in
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.rpc_staff_assign_walk_in(integer, text, integer, uuid, integer, boolean, text, text, text, text, text, text, text, date, text);
...
BEGIN
  ...
  IF p_associated_client_id IS NOT NULL THEN
    IF v_client_name IS NOT NULL OR v_client_first IS NOT NULL OR v_client_last IS NOT NULL THEN
...
REVOKE ALL ON FUNCTION public.rpc_staff_assign_walk_in(integer, text, integer, uuid, integer, boolean, text, text, text, text, text, text, text, date, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_staff_assign_walk_in(integer, text, integer, uuid, integer, boolean, text, text, text, text, text, text, text, date, text, text, text, text) TO authenticated;

-- ---------------------------------------------------------------------------
-- rpc_staff_create_reservation
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.rpc_staff_create_reservation(integer, timestamptz, timestamptz, text, text, text, uuid, integer, text, text, text, text, text, text, text, date, text);
...
BEGIN
  ...
  IF p_associated_client_id IS NOT NULL THEN
    IF v_client_name IS NOT NULL OR v_client_dni IS NOT NULL OR v_client_phone IS NOT NULL
       OR v_client_first IS NOT NULL OR v_client_last IS NOT NULL THEN
  ...
  INSERT INTO public.reservations (
    room_id, associated_client_id, client_name, client_first_name, client_last_name, client_dni, client_phone,
  ...
  VALUES (
    p_room_id, p_associated_client_id, v_client_name, v_client_first, v_client_last, v_client_dni, v_client_phone,
    v_status, p_check_in,
...
REVOKE ALL ON FUNCTION public.rpc_staff_create_reservation(integer, timestamptz, timestamptz, text, text, text, uuid, integer, text, text, text, text, text, text, text, date, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.rpc_staff_create_reservation(integer, timestamptz, timestamptz, text, text, text, uuid, integer, text, text, text, text, text, text, text, date, text, text, text) TO authenticated;
