-- Migration 62: Walk-in (check-in directo) con fork persona/empresa
--
-- Aplica al rpc_staff_assign_walk_in el mismo modelo de la migracion 61 (company_passengers):
...
--
-- Reemplaza (y deja una sola version de) rpc_staff_assign_walk_in. Dropea las dos firmas previas
-- posibles segun el camino de migracion:
--   * migracion 57: ...p_guest_name, p_guest_dni... (modo manual/asociado viejo)
--   * migracion 60 (guest_spine_walk_in): ...p_client_phone, p_guest_id... (flujo unico, superado)
-- asi un replay desde cero termina con UNA sola funcion (la del fork) y sin overloads.
...
DROP FUNCTION IF EXISTS public.rpc_staff_assign_walk_in(integer, text, integer, uuid, integer, boolean, text, text, text, text, text, date, text, text, text, text, text, uuid);
