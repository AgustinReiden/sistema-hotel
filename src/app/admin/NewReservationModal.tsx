...
export default function NewReservationModal({
  ...
  useEffect(() => {
    if (!isOpen || form.mode !== "person" || form.guestId) {
      setDniMatch(null);
      return;
    }
    const norm = normalizeDni(form.clientDni);
    if (norm.length < 6) {
      setDniMatch(null);
      return;
    }
    let active = true;
    const timer = setTimeout(async () => {
      const found = await searchGuestsAction(form.clientDni.trim());
      if (!active) return;
      const match = found.find((g) => normalizeDni(g.client_dni ?? "") === norm) ?? null;
      setDniMatch(match);
    }, 400);
    return () => {
      active = false;
  ...
  }, [isOpen, form.mode, form.guestId, form.clientDni]);

  const applyGuest = (entry: GuestDirectoryEntry) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4 bg-slate-900/50 backdrop-blur-sm">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full max-w-2xl mx-auto overflow-hidden animate-in fade-in zoom-in-95 duration-200 text-left max-h-[92dvh] sm:max-h-[88dvh] overflow-y-auto">
